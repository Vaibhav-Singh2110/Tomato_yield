...
for country in data['Country'].unique():
    ...
    try:
        ...
        print(f'R-squared for {country}: {e}')
        r2 = None
    ...
    new_data = pd.DataFrame({'Year': [2024], 'Country': [country]})
    predicted_yield = model.predict(new_data[['Year']])
    predicted_data = predicted_data.append({'Country': country, 'Predicted_Yield': predicted_yield[0]},
                                           ignore_index=True)

# Save the predicted data to a new CSV file
predicted_data.to_csv('predicted_yields_by_country.csv', index=False)
...
for country, historical_yield in historical_yields.items():
    ...
    plt.title(f'Yields for {country} (Historical and Predicted for 2024)')
    plt.xlabel('Year')
    plt.ylabel('Yield')
    plt.legend()
    plt.show()
